            ...
            barcodes.forEach(barcode => {
                if (barcode.rawValue.length === 10) {
                    ...
                    if (isDuplicate) {
                        ...
                        detectedBarcodes.push(barcode.rawValue);
                        const resultDiv = document.createElement('div');
                        resultDiv.textContent = "Lido com sucesso: " + barcode.rawValue.slice(-4); // Obtém os últimos 4 dígitos
                        resultDiv.classList.add('success');
                        barcodeResults.appendChild(resultDiv);
                        finishButton.style.display = 'block';
                        playSuccessSound();
...
function finishCollection() {
    ...
    detectedBarcodes.forEach(barcode => {
        const resultDiv = document.createElement('div');
        resultDiv.textContent = "Lido com sucesso: " + barcode.slice(-4); // Obtém os últimos 4 dígitos
        resultDiv.classList.add('error'); // Adiciona a classe 'error' para tornar o texto vermelho
        barcodeResults.appendChild(resultDiv);
    });
    window.location.href = 'resumo.html';
}
